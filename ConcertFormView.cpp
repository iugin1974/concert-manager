...
void ConcertFormView::addPiece(std::vector<MusicalPiece> &program)
{
...
void ConcertFormView::deletePiece(std::vector<MusicalPiece> &program)
{
...
void ConcertFormView::editPiece(std::vector<MusicalPiece> &program)
{
...
void ConcertFormView::addMusician(std::vector<Musician>& musicians) {
Musician m;
				if (runMusicianForm(nullptr, m))
					musicians.push_back(m);
}

void ConcertFormView::editMusician(std::vector<Musician>& musicians) {
...
void ConcertFormView::deleteMusician(std::vector<Musician>& musicians) {
int idx = runChoiceForm(musicians);
				if (idx == -1)
					return;
				bool del = confirmDialog(stdscr);
...
void ConcertFormView::addRehearsal(std::vector<Rehearsal>& rehearsals) {
...
void ConcertFormView::editRehearsal(std::vector<Rehearsal>& rehearsals) {
...
void ConcertFormView::deleteRehearsal(std::vector<Rehearsal>& rehearsals) {
...
std::vector<std::string> menuTitles = {
        "File", "Musician", "Piece", "Rehearsal", "Tools"
    };
    ...
    NcursesMenuBar menubar(menuTitles, menuItems);
    std::pair<int, int> result = menubar.activate();
...
else if (result.first == 1) { // Musician
switch (result.second) {
			case 0:	{ addMusician(musicians); break; }
			case 1:	{ editMusician(musicians); break; }
			case 2: {  deleteMusician(musicians); break; }
}
}
else if (result.first == 2) { // Piece
switch (result.second) {
			case 0: { addPiece(program); break; }
			case 1: { editPiece(program); break; }
			case 2: { deletePiece(program); break; }
}
}
else if (result.first == 3) { // Reearsal
switch (result.second) {
			case 0: { addRehearsal(rehearsals); break; }
			case 1: { editRehearsal(rehearsals); break; }
			case 2: { deleteRehearsal(rehearsals); break; }
}
}
else if (result.first == 4) { // Tools
switch (result.second) {
			case 0: { editComment(comment); break; }
