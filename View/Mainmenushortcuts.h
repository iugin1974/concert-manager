...
// Ritorna il comando se sì, std::nullopt altrimenti.
inline std::optional<MenuCommand> matchSingleKeyShortcut(int ch) {
    auto it = mainMenuSingleKeyShortcuts.find(static_cast<char>(ch));
    if (it == mainMenuSingleKeyShortcuts.end()) {
        return std::nullopt;
    }
    return it->second;
}
