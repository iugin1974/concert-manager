...
std::optional<std::vector<std::string>> ScoreSelectView::show(
	...
	while (true) {
		...
		if (std::isalpha(ch)) {
			...
			for (size_t i = 0; i < availablePaths.size(); ++i) {
				std::string filename =
						fs::path(availablePaths[i]).filename().string();
		...
		case ' ': {
			...
			if (std::find(selectedFiles.begin(), selectedFiles.end(), chosen)
					== selectedFiles.end()) {
		...
		case 10: // Enter
			if (selectedFiles.size() == 0) {
				...
				if (std::find(selectedFiles.begin(), selectedFiles.end(),
						chosen) == selectedFiles.end()) {
					selectedFiles.push_back(chosen);
				}
			}
...
void ScoreSelectView::draw(const std::vector<std::string> &availablePaths,
		const std::vector<std::string> &selectedFiles) {
	clear();
	int maxDisplay = windowHeight - 2;
	mvprintw(0, 0,
			"Select a score file (SPACE: select - ENTER: finish - ESC: cancel):");

	for (int i = 0; i < maxDisplay && i + offset < (int) availablePaths.size();
			++i) {
		int index = i + offset;
		bool isSelected = std::find(selectedFiles.begin(), selectedFiles.end(),
				availablePaths[index]) != selectedFiles.end();

		if (index == currentSelection)
			attron(A_REVERSE);

		std::string filename =
				fs::path(availablePaths[index]).filename().string();
